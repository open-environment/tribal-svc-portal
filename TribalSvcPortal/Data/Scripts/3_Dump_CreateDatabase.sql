...
/*
	...
	drop table [T_OD_DUMP_ASSESSMENT_RESTORE];
	drop table [T_OD_DUMP_ASSESSMENT_CLEANUP];
...
Scaffold-DbContext -UseDatabaseNames "Server=.\SQLEXPRESS;Database=TRIBAL_SVC_PORTAL;Trusted_Connection=True;" Microsoft.EntityFrameworkCore.SqlServer -OutputDir Data/Models -t T_OD_DUMP_ASSESSMENT_RESTORE, T_OD_DUMP_ASSESSMENT_CLEANUP, T_OD_DUMP_ASSESSMENT_CONTENT, T_OD_DUMP_ASSESSMENT_DOCS, T_OD_DUMP_ASSESSMENTS, T_OD_SITES, T_OD_REF_WASTE_TYPE, T_OD_REF_WASTE_TYPE_UNITS, T_OD_REF_WASTE_TYPE_CAT_CLEANUP, T_OD_REF_WASTE_TYPE_CAT, T_OD_REF_CLEANUP_ASSETS, T_OD_REF_THREAT_FACTORS, T_OD_REF_DATA, T_OD_REF_DATA_CATEGORIES, T_OD_REF_DISPOSAL  -f -Context "ApplicationDbContextTemp"
*/
use [TRIBAL_SVC_PORTAL]
...
(
    ...
    CONSTRAINT [FK_T_OD_DUMP_ASSESSMENTS_S] FOREIGN KEY ([SITE_IDX]) REFERENCES [T_OD_SITES] ([SITE_IDX]) ON UPDATE CASCADE ON DELETE CASCADE,
...
--ALTER TABLE T_OD_DUMP_ASSESSMENTS DROP COLUMN [ACTIVE_SITE_IND]
--ALTER TABLE T_OD_DUMP_ASSESSMENTS ADD [CURRENT_SITE_STATUS] varchar(30) NULL;
--ALTER TABLE T_OD_DUMP_ASSESSMENTS ADD [CLEANED_CLOSED_DT] datetime2(0) NULL;
...
(
    ...
    CONSTRAINT [FK_T_OD_DUMP_ASSESS_DOCS_A] FOREIGN KEY ([DUMP_ASSESSMENTS_IDX]) REFERENCES [T_OD_DUMP_ASSESSMENTS] ([DUMP_ASSESSMENTS_IDX]) ON UPDATE CASCADE ON DELETE CASCADE,
...
(
    ...
    CONSTRAINT [FK_T_OD_DUMP_ASSESS_CNT_A] FOREIGN KEY ([DUMP_ASSESSMENTS_IDX]) REFERENCES [T_OD_DUMP_ASSESSMENTS] ([DUMP_ASSESSMENTS_IDX]) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT [FK_T_OD_DUMP_ASSESS_CNT_D] FOREIGN KEY ([REF_WASTE_TYPE_IDX]) REFERENCES [T_OD_REF_WASTE_TYPE] ([REF_WASTE_TYPE_IDX]),
    CONSTRAINT [FK_T_OD_DUMP_ASSESS_CNT_U] FOREIGN KEY ([UNIT_MSR_IDX]) REFERENCES [T_PRT_REF_UNITS] ([UNIT_MSR_IDX])
);


CREATE TABLE [T_OD_DUMP_ASSESSMENT_CLEANUP] 
(
	[DUMP_ASSESSMENT_CLEANUP_IDX] uniqueidentifier NOT NULL DEFAULT NEWID(),
	[DUMP_ASSESSMENTS_IDX] uniqueidentifier NOT NULL,
	[REF_WASTE_TYPE_CAT] [VARCHAR](100) NOT NULL,
	[REF_ASSET_NAME] [VARCHAR](100) NOT NULL,
	[CLEANUP_COST] decimal(10,2) NULL,
    CONSTRAINT [PK_T_OD_DUMP_ASSESS_CLEANUP] PRIMARY KEY ([DUMP_ASSESSMENT_CLEANUP_IDX]),
    CONSTRAINT [FK_T_OD_DUMP_ASSESS_CLEAN_A] FOREIGN KEY ([DUMP_ASSESSMENTS_IDX]) REFERENCES [T_OD_DUMP_ASSESSMENTS] ([DUMP_ASSESSMENTS_IDX]) ON UPDATE CASCADE ON DELETE CASCADE
);


CREATE TABLE [T_OD_DUMP_ASSESSMENT_RESTORE] 
(
	[DUMP_ASSESSMENT_RESTORE_IDX] uniqueidentifier NOT NULL DEFAULT NEWID(),
	[DUMP_ASSESSMENTS_IDX] uniqueidentifier NOT NULL,
	[RESTORE_CAT] [VARCHAR](20) NOT NULL,
	[RESTORE_ACTIVITY] [VARCHAR](100) NOT NULL,
	[RESTORE_COST] decimal(10,2) NOT NULL,
	[MODIFY_USER_ID] nvarchar(450) NULL,
	[MODIFY_DT] datetime2(0) NULL,
    CONSTRAINT [PK_T_OD_DUMP_ASSESS_RESTORE] PRIMARY KEY ([DUMP_ASSESSMENT_RESTORE_IDX]),
    CONSTRAINT [FK_T_OD_DUMP_ASSESS_RESTORE_A] FOREIGN KEY ([DUMP_ASSESSMENTS_IDX]) REFERENCES [T_OD_DUMP_ASSESSMENTS] ([DUMP_ASSESSMENTS_IDX]) ON UPDATE CASCADE ON DELETE CASCADE
);